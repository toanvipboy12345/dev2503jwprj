...
(function() {
    ...
    $(document).ready(function() {
        console.log('Document ready - transport.js loaded');

        const modal = $('#editTransportModal');
        modal.on('hidden.bs.modal', function() {
            $('.edit-transport-btn').first().focus();
            modal.removeAttr('aria-hidden');
        });

        $(document).on('click', '.edit-transport-btn', function() {
        ...
        $('#saveTransportBtn').on('click', function() {
            const form = $('#editTransportForm')[0];
            const formData = new FormData(form);
            const transportId = $('#transportId').val();

            $.ajax({
                url: '/admin/transport/edit/' + transportId,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                success: function(response) {
                    console.log('Cập nhật phương thức vận chuyển thành công:', response);
                    ...
                    if (row.length) {
                        ...
                        const editButton = row.find('.edit-transport-btn');
                ...
                error: function(xhr) {
                    ...
                    try {
                        ...
                        errorMessage = errorObj.message || 'Có lỗi xảy ra khi cập nhật phương thức vận chuyển';
                    } catch (e) {
                        errorMessage = xhr.responseText || 'Có lỗi xảy ra khi cập nhật phương thức vận chuyển';
                    }
                    console.error('Lỗi khi cập nhật phương thức vận chuyển:', errorMessage);
                    $('#modal-error').removeClass('d-none').text(errorMessage);
                }
            });
        });

        $(document).on('click', '.delete-transport-btn', function() {
            const transportId = $(this).data('id');
            $('#confirmDeleteBtn').data('id', transportId);
        });

        $('#confirmDeleteBtn').on('click', function() {
            const transportId = $('#confirmDeleteBtn').data('id');
            $.ajax({
                url: '/admin/transport/delete/' + transportId,
                type: 'POST',
                data: { _csrf: $('input[name="_csrf"]').val() },
                success: function(response) {
                    console.log('Xóa phương thức vận chuyển thành công:', response);
                    $('#deleteConfirmationModal').modal('hide');
                    $(`tr[data-id="${transportId}"]`).remove();
                    $('#successModal').modal('show');
                    setTimeout(() => $('#successModal').modal('hide'), 3000);
                },
                error: function(xhr) {
                    ...
                    try {
                        const errorObj = JSON.parse(xhr.responseText);
                        errorMessage = errorObj.message || 'Có lỗi xảy ra khi xóa phương thức vận chuyển';
                    } catch (e) {
                        errorMessage = xhr.responseText || 'Có lỗi xảy ra khi xóa phương thức vận chuyển';
                    }
                    console.error('Lỗi khi xóa phương thức vận chuyển:', errorMessage);
                    $('#modal-error').removeClass('d-none').text(errorMessage);
                }
            });
        });
